...
public class DatabaseHelper extends SQLiteOpenHelper {
	...
	public void AddEmployees(List<Employee> employ) {
		SQLiteDatabase db = this.getWritableDatabase();
		db.beginTransaction();
		try {
			for (int i = 0, size = employ.size(); i < size; i++) {
				ContentValues cv = new ContentValues();
				cv.put(colName, employ.get(i).getName());
				cv.put(colAge, employ.get(i).getAge());
				cv.put(colDept, employ.get(i).getDept());
				db.insert(employeeTable, colName, cv);
			}
	...
	public void selectEmployee(){
		SQLiteDatabase db = this.getWritableDatabase();
		Cursor cur = db.rawQuery("Select * from " + employeeTable+"where ", null);
		int x = cur.getCount();
	}
