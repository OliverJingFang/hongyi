...
public class AddEmployee extends Activity {
    ...
    public void btnAddEmp_Click(View view) {
        ...
        try {
            /*
             * Spannable spn = txtAge.getText(); String name =
             * txtName.getText().toString(); int age =
             * Integer.valueOf(spn.toString()); int deptID =
             * Integer.valueOf((int) spinDept.getSelectedItemId()); Employee emp
             * = new Employee(name, age, deptID); employList=new
             * ArrayList<Employee>(); for (int i = 0; i <= 20000; i++) {
             * employList.add(emp); }
             */
            dbHelper.AddEmployees(employList);
        }
        catch (Exception ex) {
            ok = false;
            CatchError(ex.toString());
